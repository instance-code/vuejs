...
export default {
  ...
  methods: {
    ...
    deleteU(position) {
        this.numberOfAdd--;
        const removeIndex=this.arrayAdd.map(function(item) { return item.position; }).indexOf(position);
    ...
    deleteFromParams(position){
        ...
        this.numberOfAdd--;
        const removeIndex=this.params.map(function(item) { return item.position; }).indexOf(position);
        this.params.splice(removeIndex, 1);
        }
    },
    save(){
        for(var i=0 ;i<this.arrayAdd.length;i++){
            if(this.arrayAdd[i]!={}&&(this.arrayAdd[i].firstName!=""||this.arrayAdd[i].lastName!="")){//check data
